...
export default function ProductCard({ product, onAddToCart }: ProductCardProps) {
  return (
    <motion.div
      whileHover={{ y: -6 }}
      className="relative group bg-slate-800/90 border border-slate-700/80 hover:border-brand-primary hover:shadow-[0_10px_30px_rgba(251,191,36,0.2)] rounded-2xl sm:rounded-[24px] overflow-hidden p-3 sm:p-5 transition-all duration-300 flex flex-col justify-between h-full"
      id={`product-card-${product.id}`}
    >
      {product.badge && (
        <div className="absolute top-0 right-0 z-10">
          <div className="bg-brand-primary text-brand-bg text-[8px] sm:text-[10px] font-black px-2 py-0.5 sm:px-3 sm:py-1 uppercase rounded-bl-lg sm:rounded-bl-xl tracking-wider shadow-md">
            {product.badge}
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col items-center text-center flex-1">
        <div className="relative w-full h-28 sm:h-40 mb-3 sm:mb-4 flex items-center justify-center group-hover:scale-105 transition-transform duration-500 rounded-xl overflow-hidden bg-slate-900/60 border border-slate-700/50">
          <div className="absolute inset-0 rounded-full blur-2xl transition-colors bg-brand-primary/10 group-hover:bg-brand-primary/20" />
          {product.image ? (
            <img 
              src={product.image} 
              alt={product.name}
